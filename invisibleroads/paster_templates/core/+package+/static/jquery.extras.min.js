function getTipByName(b){var d={};b.each(function(){d[this.name]=this.title});return d}function setTipByName(b,d){var a=!1;b.each(function(){var b=this.name,e=d[b];e&&($(this).prop("title","<span class=error>"+e+"</span>").tooltip().show(),a||($("[name="+b+"]").focus().select(),a=!0))})}
...
$.fn.prepareForm=function(){function b(){var a=$(this).find(".replace");a.length&&(a.siblings("input").prop("disabled",!0).hide(),a.show())}function d(){var a=$(this).find(".replace");a.length&&(a.hide(),a.siblings("input").prop("disabled",!1).show())}return $(this).each(function(){var a=$(this),c=a.find("[name]"),e=getTipByName(c);c.tooltip({position:"center right",tipClass:"formTip",events:{file:"focus mouseenter,blur mouseleave"},onBeforeShow:function(){var b=this.getTrigger(),c=b.prop("title");
c&&(this.getTip().html(c),b.prop("title",""));this.getConf().offset=[0,a.offset().left+a.outerWidth()-(b.offset().left+b.outerWidth())+10]},onHide:function(){var a=this.getTrigger();a.prop("title",e[a.prop("name")])}});a.ajaxForm({onSubmit:function(){if(!a.find("[name=token]").length)try{a.append('<input name=token type=hidden value="'+token+'">')}catch(b){}a.find(".save").prop("disabled",!0)},onResponse:function(b){b.isOk?a.trigger("onSuccess",[b]):b.message?alert(b.message):setTipByName(c,b.errorByID);
a.find(".save").prop("disabled",!1)}}).bind({showAdd:d,showEdit:b}).find(".replace").click(function(){d.apply($(this).parents("form"),[null])})})};$.fn.prepareOverlayForm=function(){return $(this).prepareForm().each(function(){var b=$(this),d=b.find("[name]");b.overlay({mask:{color:"#000",loadSpeed:0},onLoad:function(){setTipByName(d,{});d.first().focus().select()},onClose:function(){$(".formTip").hide()},closeOnClick:!1});b.bind("onSuccess",function(){b.overlay().close()})})};
