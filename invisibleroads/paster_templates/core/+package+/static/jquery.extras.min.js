...
"="+f[key]);window.location=g+"?"+e.join("&")}}}}})};function autoCompleteProxy(b,c,a,d,e){var f=b.find("[name="+c+"]"),g=f.prev();b.bind("showAdd",function(){f.val(d);g.val(e)}).bind("showEdit",function(a,b){g.val($.trim(b.find("."+c).text()))});$.getJSON(a,function(a){a.isOk?g.autoComplete({formatResult:function(a){return"<span rel="+a[0]+">"+a[1]+"</span>"},sourcePacks:a.items}).bind("itemSelect.ac",function(a,b){f.val($(b).children("span").attr("rel"))}):alert(a.message)})}
